...
| **The starter dialect**: different colours, photos and ideas, the same square controls, open sections, stacked section headers, condensed display face and 4px rhythm — because the template's radius, spacing, control and motion scales were never touched | Never; the structural scales come from the direction like the palette does |
| **Invented promises as fact**: a remake guarantee, a signed box, a free consultation written as the business's policy when the client never offered it | Never; mark it as a proposal and list it for confirmation |
...
| **The house skeleton**: estimator hero → process steps → one price table → form, no photograph, whatever the industry — the page every rule in this skill passes | Never as a default; the sections come from the content (`interaction-depth.md` §3b) |
| **Correct but forgettable**: an R2 first viewport with a medium headline, a thin diagram and a form row — every rule met, no bold move | Never on a Persuade page; R2 limits the number of loud things, not their existence (`expression-register.md` §4b) |
| **The estimator with no estimate**: a working interaction whose answer is `[n] days`, "price after review" or a dash | Never when price or time is the question; sample rates, labelled |
