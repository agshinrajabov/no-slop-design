...
- [ ] The bold move is not poster type by habit (`--display-ratio` recorded; a type-scale streak in the log is broken), and generated or stock imagery has a light, surface and palette taken from this direction, not the tool's wooden-table-and-window default (`--image-look` recorded).
