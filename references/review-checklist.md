...
- [ ] The opening skeleton and the layout, controls and type class come from the market line; the first viewport opens the way the category opens, the tool sits there only in a `product` or `tight` language; every departure is under "Departs on:" (`departure-unwritten` clean); `--opening --market-opening` recorded.
- [ ] Design language (`design-language.md` §8): `Market:` measured from ≥ 3 competitor pages (`shoot.py --profile`, or by eye with the pages named); `Design language:` written before tokens; every departure names its axis and the brief's reason, ≤ 3 of them; the built page's measured `language` line matches the declared one or the difference is explained (`design-language-unrecorded` / `-unmeasured` clean).
- [ ] Over-correction check (§8): every style signal traceable to an attribute, a content type or the market line; ≤ 2 trend signals stacked that the market does not speak. A genre worn whole (its untouched default) is the tell, not the genre.
...
- [ ] Every warning in `design/convergence-warnings.json` was either acted on (direction changed, plan re-run) or answered under "Convergence overrides" in `DESIGN.md` with a reason tied to this content (`convergence-unanswered` clean). "Kept deliberately" is not a reason. The finished page was recorded with `design_log.py add … --record design/convergence-warnings.json`, so a collision with a run in progress is answered too.
...
- [ ] Not the skill's own tell: dark surface + serif display + fact table + one button. If the page could be swapped with another industry's page by changing the nouns, the direction failed. Beside its real competitor's first screen the page reads as the same world; beside the skill's last page for another industry it does not.
