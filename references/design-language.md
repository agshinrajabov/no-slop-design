...
| A market that moves (`motion: scene`) | The page owes one scene at R3 and above — a scroll-driven sequence, a canvas, an image sequence, the film — built as progressive enhancement over a static, indexable page, with the reduced-motion path and the no-JS render intact (`expression-register.md` §5, §7). Ten 1.23 pages came out `functional` against three markets that said `scene`: the skill's caution, not the brief's. Dropping to `functional` in such a market is a departure and needs the brief's reason (device floor, budget, a visitor in a hurry) under "Departs on:" |
