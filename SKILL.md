...
14. **Converge on the market, diverge from the skill.** The page speaks its category's language, not this skill's:
    ...
    below relative to that language (§5 there): a genre is never slop, its untouched default is. Then run
...
| `python3 scripts/design_log.py check` · `add --project … --register … --hue … --display … --structure … --language … --market-language … --screenshot page.png` · `measure page.png` | cross-project convergence: what the last few directions looked like, which axes must differ now, the same design language on unrelated industries, a page far from its market; surface polarity measured from a full-page screenshot |
